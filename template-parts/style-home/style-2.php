<div class="utao styletwo">
					<div class="uta">
						<div class="imgu">
							<a rel="<?php the_id(); ?>" class="series" href="<?php the_permalink() ?>" title="<?php the_title(); ?>"><?php echo GOV_manga::get_post_thumbnail(get_the_ID(),'medium', 130, 100); ?>
						...
						<div class="luf">
							...
							<?php if($serieslatest){
								...
								foreach ($serieslatest as $key => $value) {
									echo '<li><a href="'.chapter_url($value['permalink']).'">'.GOV_lang::get('widget_chapter_label').' '.$value['chapter'].'</a><span>'.human_time_diff($value['time'], current_time('timestamp')) . " ".GOV_lang::get('ago').'</span></li>';
								}
								echo '</ul>';
							} ?>
						</div>
